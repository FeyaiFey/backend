...
def create_user(user: UserCreate, session: Session = Depends(get_session)):  # UserCreate 是前端传给后端的数据
    ...
    if user.code == 68241373:
        new_user = User(email=user.email, username=user.username, hashed_password=hashed_password,role_id=1,file_name="avatar.jpg")   # 与后端数据库建立映射
    else:
        new_user = User(email=user.email, username=user.username, hashed_password=hashed_password,role_id=2,file_name="avatar.jpg")
